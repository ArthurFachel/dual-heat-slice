...
export PYTHON_BIN
# Export arrays without lossy string joining: each worker sources a shell-safe
# declaration generated by bash itself, preserving spaces and empty arguments.
EXTRA_ARGS_FILE="$(mktemp)"
declare -p EXTRA_ARGS > "${EXTRA_ARGS_FILE}"
export EXTRA_ARGS_FILE
trap 'rm -f "${STAGE_LIST_FILE}" "${EXTRA_ARGS_FILE}"' EXIT

run_one_stage_wrapper() {
    # shellcheck disable=SC1090
    source "${EXTRA_ARGS_FILE}"
    run_one_stage "$1" "${EXTRA_ARGS[@]}"
}
